...
if (authWarning) {
  if (currentRole === "student") {
    authWarning.textContent =
      "Student access is read-only. You can view classes and subjects, but you cannot create, edit, or delete records.";
    authWarning.classList.remove("hidden");
  } else if (currentRole === "teacher") {
    authWarning.textContent =
      "User access can manage classes and subjects, while the users and exams areas remain informational.";
    authWarning.classList.remove("hidden");
  }
}
...
function syncPageFromHash() {
  const allowedPages =
    currentRole === "admin"
      ? ["classes", "subjects", "users", "fees", "exams"]
...
function renderSubjects() {
  ...
  subjects.forEach((subject) => {
    ...
    label.className =
      "flex items-center gap-2 rounded-lg border border-slate-300 p-2";
...
function renderClasses() {
  ...
  schoolClasses.forEach((schoolClass) => {
    ...
    editButton.className =
      "mr-2 rounded-lg border border-slate-300 px-3 py-1 text-xs";
    ...
    editButton.addEventListener("click", () => {
      ...
      const selected = new Set(
        schoolClass.subjects.map((subject) => String(subject.id)),
      );
      document.querySelectorAll(".subject-checkbox").forEach((checkbox) => {
