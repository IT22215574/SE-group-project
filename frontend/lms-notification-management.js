...
async function initNotificationsModule() {
    ...
    if (currentRole === "teacher" || currentRole === "admin") {
        ...
        publishResultsBtn.addEventListener("click", async () => {
            ...
            if (!classID) {
                alert("Please select a class from the filter dropdown to publish its results.");
                return;
            }
            // For simplicity, we'll ask for subject to publish or just publish all for class? 
            // The backend expects classID and subject. Let's just ask user for subject or adapt it.
            const subject = prompt("Enter subject name to publish (e.g. Mathematics):");
            if (!subject) return;
